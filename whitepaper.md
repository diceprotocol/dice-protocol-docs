...
- **Fee accounting**: Each request charges a flat fee (0.000055 ETH). Fees accrue in the contract and are withdrawable to a designated vault address by the admin.

### 2.3 Tyche Keeper

Tyche is a Rust-based off-chain service that:

1. **Monitors the blockchain** for `Requested` events via JSON-RPC polling.
...
4. **Maintains state** in a SQLite database (request tracking, last-processed block, etc.).
5. **Exposes an API** on port 34000 for monitoring and debugging.

Tyche runs as a systemd service with automatic restart on failure.
...
|------|---------|----------|---------|
| Admin | `0x4ACD...` | Cold | Contract admin (fee changes, withdrawals, provider management) |
| Vault | `0x918E...` | Cold | Fee recipient (receives withdrawn fees) |
| Keeper | `0x8741...` | Hot | Submits reveal transactions, funded with gas ETH |
...
The final random number combines three sources:

```
randomNumber = Keccak256(userRandomness ‖ providerContribution ‖ blockHash)
```
...
- **blockHash**: The block hash of the block in which the request was included. Unknown to both parties at request time (assuming neither can control block production).

This three-way combination ensures that no single party can bias the outcome, provided at least one party is honest.
...
CommitmentMetadata {
    ...
    chain_length: u64,     // Total chain length (50,000)
}
```
...
| Operation | Gas Cost | ETH Cost (at 0.051 gwei) |
|-----------|----------|--------------------------|
| Request (consumer) | ~125,000 | 0.0000064 |
| Reveal (keeper) | ~200,000 | 0.0000102 |
| Callback | ~50,000 | 0.0000026 |
| **Total per request** | ~375,000 | ~0.000019 |

The keeper spends ~0.000019 ETH per request in gas. At a 0.000055 ETH fee, the **gross margin is ~65%** (0.000036 ETH profit per request).

### 4.3 Chain Renewal

Each hash chain has 50,000 entries. At current usage projections (100 requests/day), a chain lasts ~500 days. When the chain is exhausted, the admin registers a new commitment via `registerFor()` and the keeper is configured with the new seed.
...
- Computes `randomNumber = Keccak256(userContribution ‖ providerContribution ‖ blockHash)`
...
2. **Block watching**: Polls for new blocks at 100-block batches, filtering for `Requested` events.
...
*Dice Protocol is developed and maintained as independent infrastructure for Robinhood Chain. No third-party oracle dependencies.*
